...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['action'] == 'update') {
    ...
    $sql = "UPDATE deadline_materials SET course_id = ?, title = ?, type = ?, content = ?, from_date = ?, to_date = ?, file_path = ? WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("issssssi", $course_id, $title, $type, $content, $from_date, $to_date, $file_path, $id);
